...
export const rangeFilter = (params, response) => {
    let data = response.filter((item) => {
        ...
        if (params.rangeDate && params.rangePrice && params.rangeSize) {
            return (size >= params.rangeSize.min
        ...
        if (params.rangeDate && params.rangePrice) {
            return (price >= params.rangePrice.min
                && price <= params.rangePrice.max
                && date >= new Date(params.rangeDate.min).getTime()
                && date <= new Date(params.rangeDate.max).getTime());
        }
        ...
        if (params.rangeDate) {
            return (date >= new Date(params.rangeDate.min).getTime()
                && date <= new Date(params.rangeDate.max).getTime());
        }
...
export const maxRecord = (response, params) => {
...
export const addUsdInfo = (response, currency) => {
